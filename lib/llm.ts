...
KONUM ÇOK ÖNEMLİ: Semt/mahalle/ilçe/şehir adı cümlenin HERHANGİ bir yerinde,
herhangi bir biçimde geçebilir — başta, sonda, ekli halde ("Kadıköy'deyiz",
"Kadıköydeyiz", "Moda'da", "... istiyorum Kadıköy"). Konum İstanbul semtleriyle
SINIRLI DEĞİL — kullanıcı başka bir şehir/ilçe/kasaba belirtirse (örn. "Ayvalık",
"Alaçatı", "Bodrum", "Bursa") onu OLDUĞU GİBİ yaz. Metinde bir yer adı geçiyorsa
MUTLAK bunu location alanına yaz; yalnızca hiç yer adı yoksa null bırak. Aşağıdaki
örnekteki "Kadıköy" sadece biçimi göstermek içindir — kendi çıkardığın konumu asla
örnekteki yerle değiştirme, metinde ne yazıyorsa onu yaz.
...
Örnek 2 (İstanbul dışı bir yer — konumu örnektekiyle karıştırma) — Metin: "Ayvalık'ta
sakin bir akşam geçirmek istiyorum, yorgunum"
→ {"location":"Ayvalık","target_date":"(bugün)","date_label":"bugün","companion":null,"energy":"low","time_of_day":"evening","wants_crowd":false,"budget":"medium","time_limit":null,"mobility":null,"interests":[]}`;
...
sonuçları verilecek. Görevin, kullanıcının hedef tarihinde ve konumunda GERÇEKTEN
yapılacak etkinlikleri ayıklamak:
- Tarihi hedef tarihle uyuşmayan, geçmiş veya alakasız sonuçları ele.
- Konumu hedef konumla (veya onun yakın çevresiyle) uyuşmayan sonuçları da ele —
  başka bir şehir/semtteki bir etkinliği asla önerme, emin değilsen atla.
...
// LLM ayıklaması başarısız olursa: konumla açıkça çelişen sonuçları en azından
// eleyerek düş — yanlış şehir/semt önermektense az ama doğru sonuç ver.
function locationFilteredFallback(
  items: EventLike[],
  location: string | null
): EventLike[] {
  if (!location) return items.slice(0, 3);
  const needle = location.toLocaleLowerCase("tr");
  const matches = items.filter((i) =>
    `${i.title} ${i.meta}`.toLocaleLowerCase("tr").includes(needle)
  );
...
export async function curateEvents(
  ...
  // mock modda veriler zaten şehir-bağımsız demo içeriği — olduğu gibi dön
  if (isLlmMock()) {
    return { ticketed: raw.ticketed.slice(0, 3), free: raw.free.slice(0, 3) };
  }

  // LLM ayıklaması başarısız olursa: konumla açıkça çelişen gerçek sonuçları
  // en azından eleyerek düş — yanlış şehir/semt önermektense az ama doğru sonuç ver.
  const fallback = {
    ticketed: locationFilteredFallback(raw.ticketed, parsed.location),
    free: locationFilteredFallback(raw.free, parsed.location),
  };
...
// ---------------------------------------------------------------------------
// 2b) LLM Mekan Önerileri — Google Places key yokken genel kategori yerine
// gerçek, isimlendirilmiş yerler öner (Places API (New) alternatifi)
// ---------------------------------------------------------------------------
    ...
    .array(
      z.object({
        title: z
          .string()
          .describe(
            "Gerçek, spesifik mekan/durak adı — örn. 'Şeytan Sofrası', 'Tostçular Çarşısı', 'Melin Cafe'. ASLA genel kategori adı yazma ('tarihi yer', 'kafe', 'sahil parkı' gibi)."
          ),
        meta: z
          .string()
          .describe(
            "Kısa etiket ' · ' ile: tür · bilinen fiyat ipucu · açık hava/kapalı mekan. Emin olmadığını atla, etiket kelimesi yazma."
          ),
      })
    )
    .describe("3-4 gerçek, isimlendirilmiş mekan/durak önerisi."),
});

const VENUE_SYSTEM = `Sen verilen konumu (şehir, ilçe, kasaba, semt — ne olursa olsun)
iyi bilen yerel bir rehbersin. Kullanıcının konumuna, ruh haline, bütçesine ve hava
durumuna uygun 3-4 GERÇEK ve SPESİFİK mekan/durak öner: sokak, tepe, plaj, ada, kafe,
çarşı, müze gibi.
ASLA genel kategori adı yazma ("tarihi yer", "sahil parkı", "kafe" gibi) — gerçekten
var olduğunu bildiğin, isimli yerler yaz. Örnek: Ayvalık için "Şeytan Sofrası",
"Cunda Adası", "Tostçular Çarşısı", "Melin Cafe" gibi.
Konum İstanbul dışındaysa da (küçük bir kasaba olsa bile) aynı özenle gerçek yer
adları öner — sadece İstanbul semtlerini bilmiyormuş gibi davranma.
Bir yer adından hiç emin değilsen, o yeri uydurmak yerine listeye hiç ekleme; az
ama doğru öner. Yağmurluysa kapalı mekan ağırlıklı seç.`;

export async function suggestVenues(
  parsed: ParsedQuery,
  weather: WeatherInfo
): Promise<{ title: string; meta: string }[]> {
  if (isLlmMock()) return [];
  try {
    const out = await parseStructured(
      ...
      JSON.stringify({
        konum: parsed.location,
        enerji: parsed.energy,
        butce: parsed.budget,
        zaman_dilimi: parsed.time_of_day,
    ...
    console.error("LLM mekan önerisi hatası:", err);
    return [];
  }
}
    ...
    .array(
      z.object({
        ...
        place_query: z
          ...
          .nullable()
          .describe(
            "Adım somut bir mekansa haritada aranabilir 'mekan adı, konum' biçimi — konum HER ZAMAN kullanıcının gerçek konumu olmalı, örneği kopyalama (örn. konum Ayvalık'sa 'Şeytan Sofrası, Ayvalık', İstanbul'sa 'Yoğurtçu Parkı, Kadıköy, İstanbul'). Somut mekan değilse null."
          ),
      })
    )
...
const EXPERIENCE_SYSTEM = `Sen Türkiye'nin her şehir, ilçe ve kasabasını iyi bilen
yerel bir deneyim tasarımcısısın — sadece İstanbul değil, kullanıcının belirttiği
HANGİ konum olursa olsun aynı özenle gerçek yerler biliyorsun.
...
mekanları kullan ve adım başlığında mekan adını geçir. Liste verilmemişse bile,
konumu biliyorsan kendi bilgine dayanarak GERÇEK ve SPESİFİK yer adları kullan
(örn. Ayvalık için "Şeytan Sofrası", "Cunda Adası", "Tostçular Çarşısı") — sadece
gercek_mekanlar listesi yokmuş diye genel tarife düşme. Bir yerin adından hiç emin
değilsen (uydurma riski varsa) ancak o zaman "sahil hattında bir çay bahçesi" gibi
tarif et; işletme adı asla uydurma.
Somut bir mekan kullandığın HER adımda place_query alanını mutlaka doldur (mekan
adı + kullanıcının gerçek konumu, örn. "Melin Kahve, Ayvalık") — haritada
gösterebilmemiz bu alana bağlı. Adım soyutsa (ör. "ara sokaklarda gezinme") place_query'yi null bırak, uydurma bir mekan yazma.
...
// Sunucu hangi saat diliminde çalışırsa çalışsın (ör. UTC), takvim gününü hep
// Türkiye saatine göre hesapla — gece yarısına yakın saatlerde "bugün/yarın"
// kaymasını önler.
...
const KNOWN_DISTRICTS = [
  ...
  // İstanbul dışı bilinen şehir/ilçeler (mock parser için — LLM zaten serbest metinden çıkarır)
  "ayvalık", "alaçatı", "çeşme", "bodrum", "bursa", "izmir", "ankara",
  "antalya", "fethiye", "kaş", "assos", "foça", "urla", "datça",
];
